...
namespace App\Modules\WeeklyScheduleSt\Admin\Controllers;
...
use App\Modules\WeeklyScheduleSt\Admin\Models\WeeklyScheduleSt;
...
class WeeklyScheduleStManager extends Controller
{
    public function AddView()
    {
        if (Auth::guard('students')->check() == true) {
            ...
            foreach ($TrainingDays as $item) {
                ...
                foreach ($TrainingHours[$item->id] as $item2) {
                    $Schedule[$item2->id] = Schedule::where('week_day', $item2->week_day)
                        ...
                        ->where('class_id', $studentsAdmin->class_id)
                        ->first();
            ...
            return view($this->GetPreView() . 'Add', compact('studentsId', 'studentsAdmin', 'SchoolSelect', 'assignSelect', 'WeekDayNumber', 'timeNow', 'TrainingDays', 'TrainingHours', 'Schedule', 'classSelect', 'teacher_assign', 'lessonSelect'));
        } else {
