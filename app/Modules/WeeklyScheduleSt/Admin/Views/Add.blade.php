...
@section('content')
            ...
            <div class="row match-height">
                <div class="col-12 col-sm-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
                    <form id="form" class="form form-vertical" name="form" method="post"
                        ...
                        <input type="hidden" value="{{$studentsId}}" name="student_id" id="teacher_id">
                                                    ...
                                                    <section class="plan cf" id="plan_{{$item->week_day}}">
                                                        ...
                                                        @foreach($TrainingHours[$item->id] as $item2)
                                                            @if($Schedule[$item2->id])
                                                                @if($item2->start_time <= $timeNow && $item2->end_time >= $timeNow && $item2->week_day == $WeekDayNumber)
                                                                    ...
                                                                    <span class="spanWeek">
                                                                        ...
                                                                        <label
                                                                            ...
                                                                            <span style="font-size: 11px;display: inline-block;height: 30px;width: 100%;line-height: 30px;float: right">
                                                                ...
                                                                @else
                                                                    <span class="spanWeek">
                                                                <input disabled type="radio" name="week[{{$item->week_day}}]" id="free{{$item2->id}}" value="{{$item->week_day}}**{{$item2->time_number}}" class="radioWeek"><label class="free-label four col" for="free{{$item2->id}}" data-toggle="popover" data-original-title="از ساعت {{$item2->start_time}}" data-content="تا ساعت {{$item2->end_time}}"  data-trigger="hover" data-placement="top">
                                                                        <span style="font-size: 11px;display: inline-block;height: 30px;width: 100%;line-height: 30px;float: right">
                                                                                <span> زنگ :</span> <b>{{$item2->time_number}}</b>
                                                                            </span>
                                                                            <span style="font-size: 11px;display: inline-block;height: 30px;width: 100%;line-height: 30px;float: right">
    ...
    <input type="hidden" id="studentsCur" value="{{$studentsId}}">
@stop
@section('js')
    @foreach($TrainingDays as $item)
        @foreach($TrainingHours[$item->id] as $item2)
            @if($Schedule[$item2->id])
                @if($item2->start_time <= $timeNow && $item2->end_time >= $timeNow && $item2->week_day == $WeekDayNumber)
                    <script>
                        ...
                        $(".btn-primary").attr('href' , '{{route("ClassRoomSt.Show.View", $Schedule[$item2->id]->id)}}');
                    </script>
                @endif
            @endif
        @endforeach
