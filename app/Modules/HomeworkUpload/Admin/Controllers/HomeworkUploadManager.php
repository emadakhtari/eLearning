...
namespace App\Modules\HomeworkUpload\Admin\Controllers;
...
use App\Modules\HomeworkUpload\Admin\Models\HomeworkUpload;
...
class HomeworkUploadManager extends Controller
{
    ...
    public function AddView()
    {
        if (Auth::guard('students')->check() == true) {
            ...
            $studentAdmin = Students::where('id', $studentId)
                ->first();
            $TeacherAssignd = TeacherAssign::where('class_id', $studentAdmin->class_id)
                ->first();
            $teacherId = $TeacherAssignd->teacher_id;
            ...
            $lessonList = Lesson::where('base_id', $studentAdmin->base_id)
                ->get();
            ...
            $schedule = Schedule::where('class_id' , $studentAdmin->class_id)
                ->get();
            ...
            return view($this->GetPreView() . 'Add', compact('studentId','teacherId', 'teacherAdmin', 'SchoolSelect', 'assignSelect', 'baseList', 'lessonList', 'WeekDayNumber', 'timeNow', 'schedule', 'toDateString', 'addDays'));
    ...
    public function AddTrain(Request $request)
    {
        if ($request->file) {
            if ($_FILES["file"]["size"] > 0) {
                ...
                $pathfile = public_path('/upload/students/homeworks/' . $filenamefile);
        ...
        $request->request->add(['student_id' => $studentId]);
        $request->request->add(['teacher_id' => $request->teacher_id]);
        $request->request->add(['school_id' => $studentAdmin->school_id]);
        $request->request->add(['grade_id' => $studentAdmin->grade_id]);
        $request->request->add(['base_id' => $studentAdmin->base_id]);
        $request->request->add(['class_id' => $studentAdmin->class_id]);
        ...
        $HomeworkUpload = HomeworkUpload::create($request->all());;
        $HomeworkUpload->update([
            'file' => $filenamefile,
        ]);
//
    }
    ...
    public function Table(Request $request)
    {
        ...
        $Homework = HomeworkUpload::where('student_id', $student_id)
            ->get();

        $studentSelect = Students::where('id', $student_id)
            ->first();
                                ...
                                <table class="table table-transparent mb0">
                                    <tbody>
                                        <tr style="text-align: center;background: #DBE5F4;">
                                            <th style="border: none"><div style="padding: 15px;"><b>فهرست تکالیف ارسال شده  </b></div></th>
                                        </tr>
                                    </tbody>
                                </table>
                ...
                foreach ($Homework as $item) {
                    ...
                    $output .= '
                        <tr class="text-center" id="tr_' . $item['id'] . '">
                            ...
                            <td>
                             <a download href="'.asset('upload/students/homeworks').'/'.$item['file'].'">' .  $item['title'] . '</a>
                            </td>
                ...
                swal({
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            ...
                            $.ajax({
                                type: "POST",
                                url: "' . route('HomeworkUpload.Delete') . '",
    ...
    public function Delete(Request $request)
    {
        ...
        HomeworkUpload::where('id', $trainingInfo)->delete();
        $output = '';
        $output .= '
            <script>
