...
class DeputyAuthenticateManager extends Controller
{
    ...
    public function CheckPassword(Request $request)
    {
        ...
        $user = Deputy::where('national_code', $request->get('national_code'))
            ->first();
        ...
        if ($user != null) {
            if ($validator->passes()) {
                if ($request->get('password')) {
                    ...
                    if (Auth::guard('deputy')->attempt(['national_code' => $national_codeRequest, 'password' => $passwordRequest])) {
    ...
    public function DeputyLoginView()
    {
        if (Auth::guard('deputy')->check() == true) {
            return redirect(route('list'));
        } else {
//            $pass =bcrypt('123456');
//            DB::table('deputy')->where('id', 1)->update(['password' => $pass]);
//            dd($pass);
            $setting = Setting::where('id', 1)
                ->first();
            return view($this->GetPreView() . 'DeputyLogin', compact('setting'));
        }
    }
